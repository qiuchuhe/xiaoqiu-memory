...
2. **本周**：仓位硬约束 + 统一风控参数到单一来源
3. **下周**：提取共享函数消除重复 + 回测pd导入修复
4. **长期**：策略2回测验证 + 裸except全部替换 + 除零风险修复

## 7/11 晚清理记录

- ✅ 删除 auto_trade.py（easytrader GUI自动化废墟）
- ✅ 删除 conditional_orders.py（路径过期）
- ✅ 删除 auto_buy.py / auto_buy_0611.py / easytrader_test.py（easytrader依赖）
- ✅ 新建 condition_orders.py v2.0（读取当前持仓→生成条件单卡片→用户手动设到同花顺）
- ✅ 确认：同花顺条件单 = 服务端执行，比任何本地自动化都靠谱
