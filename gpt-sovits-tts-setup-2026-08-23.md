...
metadata: 
  ...
  modified: 2026-08-28T19:27:22.222Z
---
...
**SillyTavern 配置**：settings.json 里 `tts.currentProvider = "GPT-SoVITS-V2 (Unofficial)"`，`GPT-SoVITS-V2 (Unofficial)` 块 `provider_endpoint=http://127.0.0.1:9881`，voiceMap 按角色名→voice 名（=wav 文件名）。当前许知糯→许知糯声（星瞳），其余角色→测试声 `测试`（鸣潮权重）。**改 settings.json 后 F5 刷新页面即生效，不用重启**（server 端 `/settings/get` 每次读盘，非启动缓存）。改前先备份。Edge TTS 备用保留（provider 可随时切回）。**⚠️ 8/29 坑**：① GPT-SoVITS 块内 voiceMap 曾被清空成 `{}`（顶层 voiceMap 是 Edge 遗留 zh-CN-XiaoxiaoNeural 会串音）→ 用 `settings.json.bak_gptsovits` 恢复 61 键 + 许知糯 4 键覆盖 → F5 生效。② **无声根因（已修）**：前端 `public/scripts/extensions/tts/gpt-sovits-v2.js:187` 硬编码 `streaming_mode: 'true'` → api_v2 返回**流式 WAV**（RIFF size=36 + data size=0 + 大量 `\x01\x00\x00\x00`/`\x00\x00\x00\x00` 标记块），浏览器 `<audio>` 无法解码 → 请求 200 但静默无声。修复：改成 `streaming_mode: this.settings.streaming ? 'true' : 'false'`（settings.json 里 streaming=false）→ 返回标准 WAV（RIFF size>36 + data 有真实大小，Python wave 模块可读）→ 有声。备份 gpt-sovits-v2.js.bak_streaming_0829。改 JS 后需 **Ctrl+F5 硬刷新** 强制重载前端。
